...
#FIXME: esto deberia estar en español para entender mejor cada propiedad de la factura
config = {
    **dotenv_values('.env')
}
...
def jsonToXml(json_str):
    ...
    try:
        ...
        print(f"Error al convertir json a xml: {e}")
        return None
    data = json.loads(json_str)
...
def createXml(info: Invoice, accessKeyInvoice: str):
    ...
    nsmap = {
        ...
        #root = etree.Element('factura', attrib={'id': 'comprobante', 'version': '1.0.0'}, nsmap=nsmap)
    ...
    try:
        ...
        environment.text = ''.join(config['ENTORNO'])
        ...
        contribuyente = etree.SubElement(infoTributaria, 'contribuyenteRimpe')
        contribuyente.text = str(''.join('CONTRIBUYENTE RÉGIMEN RIMPE'))
        ...
        #
        xml_string = etree.tostring(root, pretty_print=True, encoding="utf-8",  xml_declaration=True).decode()
